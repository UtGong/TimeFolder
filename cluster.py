...
def fold_time_frames(tfs, random_seeds):
    folding_list = []
    folded_list = []
    ...
    for idx, tf in enumerate(tfs):
        if idx in random_seeds:
            ...
            if calculate_total_dl(temp_time_frame) < sum(calculate_data_dl([tf]) for tf in temp_folding_list):
...
def fold_timeline(tfs, random_seeds):
    folded_tfs = fold_time_frames(tfs, random_seeds)
    if len(folded_tfs) == 1:
        return folded_tfs
    if len(folded_tfs) < len(tfs):
        random_seeds = generate_random_seeds()
        return fold_timeline(folded_tfs, random_seeds)
    return folded_tfs

def find_mdl(tfs, MAX_ITER=1):
    # TODO: random seeds
    best_tfs = tfs
    min_dl = sum(calculate_data_dl([tf]) for tf in tfs)
    for i in range(MAX_ITER):
        random_seeds = generate_random_seeds()
        folded_tfs = fold_time_frames(tfs, random_seeds)
        folded_dl = sum(calculate_data_dl([tf]) for tf in folded_tfs)
        if folded_dl < min_dl:
            min_dl = folded_dl
            best_tfs = folded_tfs
