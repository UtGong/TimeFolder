...
def main():
    ...
    csv_file_path = 'data/StnData_2022-2023_dailytemp.csv'
    date_range = ['2020-01-01', '2023-12-31']
    date_column = 'Date'
    value_column = 'Max'
    test_data = load_and_filter_data(csv_file_path, date_range, date_column, value_column)
    ...
    folder_name = create_output_folder(date_range)

    # Initial plot
    plot_initial_data(test_data, date_column, value_column, y_params, folder_name)

    # Perform hierarchical clustering
    root_node, clustering_time = perform_clustering(test_data, date_column, value_column)

    # Find best tree cut
    best_cut, treecut_time = select_best_treecut(root_node, clustering_time)

    # Merge data and plot
    merged_data = merge_data(best_cut)
    plot_merged_data(merged_data, y_params, folder_name)
...
def create_output_folder(date_range):
    folder_name = date_range[0] + "-" + date_range[1]
    output_path = os.path.join('output/StnData_max', folder_name)
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    return folder_name


def plot_initial_data(data, date_column, value_column, y_params, folder_name):
    init_plt = draw_init_line_plot(data, date_column, value_column, *y_params)
    init_plt.savefig(os.path.join(
        'output/StnData_max', folder_name, 'init.png'))


def perform_clustering(data, date_column, value_column):
    start_time = time.time()
    root_node = perform_hierarchical_clustering(data, date_column, value_column)
    elapsed_time = time.time() - start_time
    print("--- hcluster: %s seconds ---" % elapsed_time)
    return root_node, elapsed_time


def select_best_treecut(root_node, prev_elapsed_time):
    start_time = time.time()
    best_cut = find_best_treecut(root_node)
...
def plot_merged_data(data, y_params, folder_name):
    plt = draw_merged_line_plot(data, *y_params)
    plt.savefig(os.path.join('output/StnData_max', folder_name, 'merged.png'))
